(function (h) {
  function v(a) {
    a.debug && (f("callbackIfComplete()"),
      f("totalFiles: " + a.totalFiles),
      f("filesLoaded: " + a.filesLoaded));
  ...
  function w(a, c, m) {
    c.debug && (f("loadAndParseFile('" + a + "')"),
      f("totalFiles: " + c.totalFiles),
      f("filesLoaded: " + c.filesLoaded));
    null !== a && "undefined" !== typeof a && h.ajax({
      ...
      success: function (e, b) {
        c.debug && (f("Succeeded in downloading " + a + "."),
          f(e));
        x(e, c);
        m()
      },
  ...
  function x(a, c) {
    for (var m = "", e = a.split(/\n/), b = /(\{\d+})/g, d = /\{(\d+)}/g, k = /(\\u.{4})/ig, f = 0, n = e.length; f < n; f++) {
      ...
      if (0 < l.length && "#" != l.match("^#")) {
        var p = l.split("=");
        if (0 < p.length) {
          for (var l = decodeURI(p[0]).trim(), g = 1 == p.length ? "" : p[1]; -1 != g.search(/\\$/);)
            g = g.substring(0, g.length - 1),
              g += e[++f].trimRight();
          for (var q = 2; q < p.length; q++)
            g += "=" + p[q];
          g = g.trim();
          if ("map" == c.mode || "both" == c.mode)
            (p = g.match(k)) && p.forEach(function (a) {
              g = g.replace(a, y(a))
            }),
              c.namespace ? h.i18n.map[c.namespace][l] = g : h.i18n.map[l] = g;
          if ("vars" == c.mode || "both" == c.mode)
            if (g = g.replace(/"/g, '\\"'),
              z(l),
              b.test(g)) {
              var u = !0
              ...
              g.split(b).forEach(function (a) {
                !b.test(a) || 0 !== t.length && -1 != t.indexOf(a) || (u || (r += ","),
                  r += a.replace(d, "v$1"),
                  t.push(a),
                  u = !1)
              });
  ...
  h.i18n.properties = function (a) {
    ...
    a.namespace && "string" == typeof a.namespace && (a.namespace.match(/^[a-z]*$/) ? h.i18n.map[a.namespace] = {} : (f("Namespaces can only be lower case letters, a - z"),
      a.namespace = null));
    a.path.match(/\/$/) || (a.path += "/");
    ...
    c.forEach(function (c) {
      // var e = a.path + c + ".properties"; 直接套用失败 此处为我修改之前
      var e = a.path + c + '_' + a.language + ".properties"; //此处为我修改之后
  ...
  h.i18n.prop = function (a) {
    ...
    var d;
    if ("string" == typeof b) {
      for (d = 0; -1 != (d = b.indexOf("\\", d));)
        b = "t" == b.charAt(d + 1) ? b.substring(0, d) + "\t" + b.substring(d++ + 2) : "r" == b.charAt(d + 1) ? b.substring(0, d) + "\r" + b.substring(d++ + 2) : "n" == b.charAt(d + 1) ? b.substring(0, d) + "\n" + b.substring(d++ + 2) : "f" == b.charAt(d + 1) ? b.substring(0, d) + "\f" + b.substring(d++ + 2) : "\\" == b.charAt(d + 1) ? b.substring(0, d) + "\\" + b.substring(d++ + 2) : b.substring(0, d) + b.substring(d + 1);
      var k = [];
      for (d = 0; d < b.length;)
        if ("'" == b.charAt(d))
          if (d == b.length - 1)
            b = b.substring(0, d);
          else if ("'" == b.charAt(d + 1))
            b = b.substring(0, d) + b.substring(++d);
          else {
            for (e = d + 2; -1 != (e = b.indexOf("'", e));)
              if (e == b.length - 1 || "'" != b.charAt(e + 1)) {
                b = b.substring(0, d) + b.substring(d + 1, e) + b.substring(e + 1);
                d = e - 1;
                break
              } else
                b = b.substring(0, e) + b.substring(++e);
          ...
          if (e = b.indexOf("}", d + 1),
            ...
            var n = parseInt(b.substring(d + 1, e));
            !isNaN(n) && 0 <= n ? (d = b.substring(0, d),
              "" !== d && k.push(d),
              k.push(n),
              d = 0,
              b = b.substring(e + 1)) : d = e + 1
          }
        else
          d++;
      "" !== b && k.push(b);
      b = k;
      // settings.namespace ? h.i18n.map[settings.namespace][a] = k : h.i18n.map[a] = k //不知道为啥 这个地方报错 屏蔽不影响效果
    }
    // if (0 === b.length)
    //   return "";
    ...
    //   k = "string" == typeof b[d] ? k + b[d] : f && b[d] < f.length ? k + f[b[d]] : !f && b[d] + 1 < c.length ? k + c[b[d] + 1] : k + ("{" + b[d] + "}");
    return k
  }
    ;
  h.i18n.normaliseLanguageCode = function (a) {
    var c = a.language;
    if (!c || 2 > c.length)
      a.debug && f("No language supplied. Pulling it from the browser ..."),
