        ...
        <div ref="section1" class="section">
          ...
          <div class="box">
            <div v-for="(item, index) in data.BasicInfo" :key="index" class="text">
              <p>
                <span>{{ item.name }}</span>
                <el-link v-if="item.prop === 'property_clues_file'" @click="openUrl(item.value)" type="success">{{Mysubstring(item.value)}}</el-link>
...
<script lang="ts" src="./servicesInfo3.ts"></script>
