    ...
    <div class="servicesInfo-box">
      ...
      <div ref="right" :style="{ height: height + 'px' }" class="right">
        ...
        <div ref="section0" class="section">
          <span style="margin-top:0;" :class="{ act: data.rightIndex == 0 }">{{this.data.labers[0].children[0].name}}</span>
          <el-divider></el-divider>
          <div class="box">
            <div class="text">
              <p><span>{{data.auditStatus}}</span></p>
            </div>
          </div>
        </div>
        <!-- 办案事项 -->
        <div ref="section1" class="section">
          <span :class="{ act: data.rightIndex == 1 }">{{this.data.labers[0].children[1].name}}</span>
          <el-divider></el-divider>
          <div class="box">
            <div v-for="(item, index) in data.BasicInfo" :key="index" class="text">
              <p>
                <span>{{ item.name }}</span>
                <span v-if="item.prop === 'information_01'" v-html="item.value"></span>
                <el-link v-else-if="item.prop === 'information_file'" @click="openUrl(item.value)" type="success">{{Mysubstring(item.value)}}</el-link>
        ...
        <div ref="section2" class="section">
          <span :class="{ act: data.rightIndex == 2 }">{{this.data.labers[0].children[2].name}}</span>
          <el-divider></el-divider>
          <div class="box">
            <div class="text">
              <p>
