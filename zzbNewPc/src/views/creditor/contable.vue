    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-popover
            ...
            placement="bottom"
            trigger="click"
            :content="tostr(scope.row[item.prop].admin_name)"
            popper-class="table-click"
            :visible-arrow="false"
          >
            <div slot="reference" class="cell">
              <!-- <span v-for="(item2,index2) in scope.row[item.prop].admin_name" :key="index2">{{item2}}</span> -->
              {{ scope.row[item.prop].admin_name }}
            </div>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column fixed="right" :width="data.width" label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            style="color:#67C23A;"
            @click="handleSetUp(scope.$index, scope.row)"
            type="text"
            class="delete"
            size="small"
            >管理员设置</el-button
...
export default class About extends Vue {
  ...
  numberChangeValue(newVal: string, oldVal: string) {
    let self: any = this;
    if (self.data.Data != null && self.data.Data != undefined) {
      self.data.Data.forEach((item: any) => {
        if (item.debtor_number == newVal) {
          self.$refs.singleTable.setCurrentRow(item);
        }
      });
    }
  }
  ...
  //设置
  handleSetUp(index: number, row: any) {
    this.$emit("setUp", row);
  }
  //删除
