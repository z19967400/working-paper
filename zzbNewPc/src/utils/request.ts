...
import { MAINHOST, ISMOCK, conmomPrams, PAYHOST } from "@/config";
...
// 图片文件请求路径
export const imgBaseUrl = "http://file.zhaizhubang.net";
class HttpRequest {
  ...
  interceptors(instance: any, url?: string) {
    ...
    instance.interceptors.request.use(
      (config: AxiosRequestConfig) => {
        ...
        config.headers.viptokey = getToken();
        return config;
      },
      (error: any) => {
        // console.error(error);
...
const conbineOptions = (
  ...
  if (process.env.NODE_ENV === "production") {
    Bur = opts.url.indexOf("zf/") != -1 ? PAYHOST : MAINHOST;
    //去除接口前缀
