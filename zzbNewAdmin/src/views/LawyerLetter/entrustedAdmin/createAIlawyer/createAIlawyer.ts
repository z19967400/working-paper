...
import comSetTwo from './compents/stepTwo.vue'
// import pay from '@/components/pay/pay.vue'
import addForms from '../../../sass/components/editcedrior.vue'
import { getCookie } from '@/utils/common'
import selectUser from '../../../../views/finance/componets/selectUser/selectUser.vue'
import selectAdmin from '../../../../views/finance/componets/selectAdmin2/selectAdmin.vue'
...
export default class createAIlawyer extends Vue {
  ...
  mounted() {
    let qz: string = process.env.NODE_ENV === 'production' ? '/api/' : '/api/pt'
    // this.getzhaiwulist();
  ...
  getTableData() {
    return new Promise(resolve => {
      let parmas: any = {
        member_id: this.data.member_id, //用户id
        admin_id: this.data.admin_id //管理员id
      }
      if (parmas.member_id == 0) {
        this.$message.warning('请先选择用户及管理员')
        return false
      }
      Api.getTableData2(parmas).then((res: any) => {
  ...
  openShouKuang() {
    ...
    if (parmas.member_id == '' || parmas.admin_id == '') {
      this.$message.warning('请选择用户和管理员')
      return false
    }
    this.getshoukuanTableData(parmas).then(() => {
  ...
  //获取收款信息
  getshoukuanTableData(params: any) {
    return new Promise(resolve => {
      Api.getshoukuanTableData(params).then((res: any) => {
        res.data.forEach((item: any) => {
  ...
  changeDialogVisible() {
    ...
    this.getTableData().then(() => {
      if (this.data.tableData.length == 0) {
        ...
        this.parentId = 1
        this.$message.warning('该用户下未查询到债权人')
      } else {
        this.data.dialogVisible = true
      }
    })
  }
  changeDialogVisible2() {
  ...
  getAddCollection() {
    ...
    if (
      self.collectList.payee_account_name == '' &&
      self.collectList.collection_account_number == '' &&
      self.collectList.bank_full_name == '' &&
      self.collectList.bank_code == '' &&
      self.collectList.alipay_account == '' &&
      self.collectList.alipay_phone_number == ''
    ) {
      self.$message.warning('不可全为空')
      return false
    }
    let parmas: any = Object.assign({}, self.collectList)
    parmas['member_id'] = self.data.member_id
    parmas['member_vip_account_id'] = self.data.member_vip_account_id
    parmas['admin_id'] = self.data.admin_id
    Api.addgetmaney(parmas).then((res: any) => {
      if (res.state) {
    ...
    this.$confirm('您确定删除该收款信息吗?', '提示', {
      ...
      .then(() => {
        ...
        Api.collectionDelete(id).then((res: any) => {
          if (res.state) {
            this.$message.success(res.msg)
            this.getshoukuanTableData(parmas)
          } else {
            this.$message.warning(res.msg)
          }
        })
  ...
  nexttip() {
    if (this.nextpoxType == false) {
      if (this.selectUsers == '请选择用户及管理员') {
        this.$message.warning('请选择用户及管理员')
        return false
      } else if (this.selectCreditors == '请选择债权人') {
  ...
  downTempaly(url: string) {
    // if (this.selectCreditors == "请选择用户及管理员") {
    ...
    this.$confirm('提交委托后不可修改债务信息，是否确认提交?', '提示', {
      ...
      .then(() => {
        ...
        let parmas: any = {
          ...
          member_id: this.data.member_id,
          member_vip_account_id: this.data.member_vip_account_id,
          admin_id: this.data.admin_id
        }
        // eslint-disable-next-line no-console
        ...
        Api.AILawyerLetterInsert(parmas).then((res: any) => {
          if (res.state) {
            ...
            //   type: 'success'
            // })
            Api.AfterPay(res.data.pay_number).then((res: any) => {
              if (res.state) {
                this.fullscreenLoading = false
                this.$notify({
                  title: '先催后付支付成功',
                ...
                this.$router.push('/LawyerLetter/entrustList')
              } else {
                this.fullscreenLoading = false
                this.$notify.error({
                  title: '操作失败！',
  ...
  //确定选择的用户
  selectUser() {
    Api.GetMemberVipAdminsByMmeberIdV2(this.data.member_id).then((res: any) => {
      if (res.state) {
        ...
        this.selectUsers = `用户：${this.adminData.name}`
        if (res.data.member.m_type == 'VIP') {
          this.data.active = 1
        } else {
  ...
  close() {
    ...
    // this.data.member_vip_account_id = 0
    // this.data.admin_id = 0
  }
  //返回用户ID
  setUserId(val: number) {
    this.data.member_id = val
  }
  //返回管理员ID
  selectAdmin(val: number, row: any) {
    this.data.member_vip_account_id = row[0].member_vip_account_id
    this.data.admin_id = row[0].id
    if (val) {
      this.selectUsers = this.selectUsers + `  管理员：${row[0].admin_name}`
      this.data.invoiceEditType = false
    } else {
      this.$message.warning('未选择管理员')
    }
  }
}
