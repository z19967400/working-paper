...
import addForms from './components/editcedrior.vue'
...
export default class creatLaywerCase extends Vue {
  adminData: any = {
    name: '',
    type: '',
    admin_list: []
  ...
  strhandle(str: string) {
    ...
    Object.keys(newObj).forEach((key: any) => {
      if (newObj2[key] != undefined) {
        ...
        newObj[key].value = newObj2Val.substring(0, newObj2Val.lastIndexOf(','))
      }
    })
    return newObj
  }
  ...
  selectAdmin(val: number, row: any) {
    ...
    if (val) {
      this.data.selectUsers =
        this.data.selectUsers + `  管理员：${row[0].admin_name}`
  ...
  //确定选择的用户
  selectUser() {
    Api2.GetMemberVipAdminsByMmeberIdV2(this.data.member_id).then(
      (res: any) => {
        if (res.state) {
          ...
          this.data.selectUsers = `用户：${this.adminData.name}`
